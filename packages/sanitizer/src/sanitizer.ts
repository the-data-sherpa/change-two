...
function buildTransformations(source: string, sanitized: string, findings: readonly SanitizationFinding[]): TransformationRecord[] {
  ...
  const records: TransformationRecord[] = []; for (const path of listFiles(sanitized)) { if (path === "checksums.json") continue; const sourcePath = join(source, path); if (!existsSync(sourcePath) || !lstatSync(sourcePath).isFile()) continue; const sourceSha256 = sha256(readFileSync(sourcePath)); const sanitizedSha256 = sha256(readFileSync(join(sanitized, path))); if (sourceSha256 === sanitizedSha256) continue; const rules = rulesByPath.get(path); records.push({ sourcePath: path, sourceSha256, sanitizedPath: path, sanitizedSha256, rule: rules?.map((rule) => rule.id).sort().join(",") ?? "evidence-rematerialization", reason: rules?.map((rule) => rule.reason).sort().join(" ") ?? "Derived evidence was regenerated after redaction." }); } return records.toSorted((a, b) => a.sourcePath.localeCompare(b.sourcePath));
}
